...
// //   }
// //
// //   Future<bool> checkIfProductExists(String barcode, BuildContext context, {required String activeStoreId}) async {
// //     try {
// //       final ownerId = await _customerService.getOwnerId();
// //       if (ownerId == null) {
...
// //       );
// //
// //       print('CheckIfProductExists Response: success=${response?.success}, message=${response?.message}, product=${response?.data?.product?.toJson()}');
// //
// //       if (response?.success == true && response?.data?.product != null) {
// //         final product = response!.data!.product!;
// //         cartItems.add(Cart(
...
// //       print('Tax: $tax');
// //       print('Payment Method: ${paymentMethod ?? 'CASH'}');
// //
...
//   }
//
//   Future<bool> checkIfProductExists(String barcode, BuildContext context,
//       {required String activeStoreId}) async {
//     try {
//       final ownerId = await _customerService.getOwnerId();
//       if (ownerId == null) {
//         print('Owner ID is missing');
...
//
//       print(
//           'CheckIfProductExists Response: success=${response?.success}, message=${response?.message}, product=${response?.data?.product?.toJson()}');
//
//       if (response?.success == true && response?.data?.product != null) {
//         final product = response!.data!.product!;
//         cartItems.add(Cart(
...
class SaleViewModel extends BaseViewModel {
  ...
  void _addOrUpdateCartItem(ScanProduct product) {
    ...
    if (existingIndex != -1) {
      ...
      final newItem = Cart(
        ...
        subtotal: product.price ?? 0,
        availableQuantity: product.stock ?? 0,
        size: product.size ?? '0', // Ensure size is included
      );
      cartItems.add(newItem);
      print(
          "Added new cart item: ${newItem.name}, Quantity: ${newItem.quantity}, Subtotal: ${newItem.subtotal}");
  ...
  void updateItemQuantityInReview(Cart cartItem, int newQuantity) {
    final index = cartItems.indexWhere((item) => item.code == cartItem.code);
    if (index != -1) {
      if (newQuantity <= cartItems[index].availableQuantity && newQuantity > 0) {
        cartItems[index].quantity = newQuantity;
        cartItems[index].subtotal = newQuantity * cartItems[index].price;
        print(
            "Updated quantity for ${cartItems[index].name}: New Qty: $newQuantity, Subtotal: ${cartItems[index].subtotal}");
