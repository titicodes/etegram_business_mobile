...
class _AddProductScannerViewState extends State<AddProductScannerView>
    ...
    return showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          TextButton(
            onPressed: () {
              print('AddProductScannerView: Scan Another selected');
          ...
          TextButton(
            onPressed: () {
              print('AddProductScannerView: Navigating to mainNavViewRoute');
              Navigator.pop(context);
              Navigator.pushReplacementNamed(context, mainNavViewRoute);
            },
  ...
  Future<void> _navigateToAddProduct(String barcodeValue, String storeId,
    ...
    final Map<String, dynamic> arguments = {
      ...
      'externalProduct': Product(
        name: model.nameController.text.isNotEmpty
            ? model.nameController.text
            : null,
        code: barcodeValue,
        category: model.categoryController.text.isNotEmpty
            ? model.categoryController.text
            : null,
        brands: model.brandsController.text.isNotEmpty
            ? [model.brandsController.text] // Wrap the string in a list
            : null,
        size: model.sizeController.text.isNotEmpty
            ? model.sizeController.text
            : null,
        description: model.descriptionController.text.isNotEmpty
            ? model.descriptionController.text
            : null,
        imageUrl: model.productImageUrl,
      ),
      'needsImageSelection': model.productImageUrl == null,
    };

    print(
        'AddProductScannerView: Navigating to addProductViewRoute with arguments: $arguments');
    await Navigator.pushNamed(context, addProductViewRoute,
        arguments: arguments);

    if (mounted && !_isDisposed) {
      print('AddProductScannerView: Returned from AddProductView');
      setState(() => _isProcessing = false);
      await Future.delayed(const Duration(milliseconds: 500));
      if (mounted && !_isDisposed) {
        await _restartScanner();
  ...
  Widget build(BuildContext context) {
    return BaseView<ProductViewModel>(
      onModelReady: (model) {
        print(
            'AddProductScannerView: Model ready, instance: ${model.hashCode}');
        model.init();
      },
