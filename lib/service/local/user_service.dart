...
class CustomerService {
  ...
  Future<String?> getUserRole() async {
    try {
      ...
      if (accessToken == null) {
        print('CustomerService: No token found');
        return null;
      }

      // Decode JWT token to extract role
      Map<String, dynamic> decodedToken = JwtDecoder.decode(accessToken);
      dynamic role = decodedToken['role'];
      String? roleString;

      // Handle role as either String or List<dynamic>
      if (role is String) {
        roleString = role;
      } else if (role is List<dynamic> && role.isNotEmpty) {
        roleString = role.first.toString();
      } else {
        roleString = null;
      }

      print('CustomerService: Decoded user role: $roleString');
      return roleString;
    } catch (e, stackTrace) {
      print('CustomerService: Error decoding token: $e\n$stackTrace');
      return null;
    }
  }
  ...
  Future<void> updateFcmToken(String fcmToken) async {
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        print(
            'CustomerService: Failed to update FCM token, status: ${response.statusCode}');
